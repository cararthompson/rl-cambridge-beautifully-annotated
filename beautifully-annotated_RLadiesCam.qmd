...
#| output-location: column-fragment
#| echo: true
#| code-line-numbers: "9-10,11-12,15"
basic_plot +
  theme(legend.position = "none",
        text = element_text(colour = vit_c_palette["light_text"], 
                            family = "Cabin"),
        ...
        plot.subtitle = element_text(family = "Cabin", size = rel(1.1), lineheight = 1.3,
...
themed_plot +
  ...
  theme(strip.text = element_text(family = "Enriqueta",
                                  ...
                                  hjust = 0.03,
                                  margin = margin(2, 0, 0, 0, "lines"))) +
  # x and y are already set in the global ggplot() call! 
  ggtext::geom_textbox(aes(label = mean_length),
                       halign = 1, 
                       hjust = 1,
                       fill = NA,
...
themed_plot +
  ...
  scale_colour_identity() +
  ggtext::geom_textbox(aes(
    label = paste0("<span style=font-size:9pt>", 
                   dose, "mg/day</span><br>", 
                   mean_length, "mm"),
    hjust = case_when(mean_length < 15 ~ 0,
                      TRUE ~ 1),
...
Be brave - give `theme_void()` a go as your starting point! 
...
Or just fade the grid lines a tad, using a matching colour
...
TODO: illlustrate, doesn't work in our plot, but best example seen recently is Nicola Rennie's plot about Hollywood age differences

## Tee pipe

TODO: Giving the guinea pigs names half way through the pipe
...
TODO: create two geom_curves (one for min, one for max). If want to use more curvatures or have more complex data, suggest adding to dataframe then looping through. Point to useR talk on how to do that (upload video!)
